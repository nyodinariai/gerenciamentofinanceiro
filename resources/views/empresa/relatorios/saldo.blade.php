...
@section('title', 'Relatório de Saldo')
    @section('mainTitle')
        <h1>Relatório de Saldo Empresa {{ $empresa->nome}}</h1>
    @stop
    @section('breadcrumb')
        <li class="breadcrumb-item">
            <a href="{{ route('empresas.relatorios.saldo', $empresa) }}">Relatório Saldo {{ $empresa->nome}}</a>
        </li>
    @stop
@section('content')
                                ...
                                @foreach($saldo as $item)
                                    <tr>
                                        ...
                                        @if ($item->movimento_type === 'App\Models\MovimentosEstoque')
                                            ...
                                            <td>{{ $item->movimento->quantidade * $item->movimento->valor }}</td>
                                            <td>{{ $item->valor}}</td>
                                        @else
                                            <td>Financeiro</td>
                                            <td>{{ $item->movimento->tipo }}</td>
